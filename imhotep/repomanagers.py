...
class RepoManager(object):
    ...
    def add_remote(self, dirname, name, url):
        log.debug("Adding remote %s url: %s", name,
                                              url)
        self.executor("cd %s && git remote add %s %s" % (dirname,
                                                         name,
                                                         url))

    def shallow_clone(self, repo, dirname, repo_name, remote_repo, ref):
        remote_name = 'origin'
    ...
    def clone_repo(self, repo_name, remote_repo, ref):
        ...
        if self.shallow:
            log.debug("Shallow cloning: %s", ref)
            self.shallow_clone(repo, dirname, repo_name, remote_repo, ref)
